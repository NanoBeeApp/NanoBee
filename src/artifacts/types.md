...
## 核心导出 / API
- `Artifact`：完整产物(id/kind/title/cardCount/deck/chatId?/favorited/createdAt)
...
### 2026-06-15 — 增加 favorited
- **出发点**：artifacts 页加「你收藏的」tab,需要在 Artifact 上携带收藏态
- **目标**：Artifact 增加 `favorited: boolean`
- **关键决策**：作为信封字段而非单独引用类型,列表一次返回收藏态,前端可直接按它过滤
