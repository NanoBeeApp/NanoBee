...
## Responsibility
Main chat composer: auto-growing textarea, slash (`/任务` /task · `/提醒` /reminder · `/盯盘` /watch · `/早报` /daily-brief) popover, toolbar (attach / slash / voice / model) and send button. The `@` mention popover was removed along with the hardcoded watchlist.
...
- The placeholder text no longer mentions "@ 引用关注" ("@ to reference a followed item") since that feature is gone.
...
- **Motivation**: user asked to drop the chip block ("设一个提醒" / "帮我盯着这个" / "每天给我摘要" — "Set a reminder" / "Watch this for me" / "Give me a daily summary" — in a topic, and the welcome-state variant). The chips duplicated actions already reachable via slash commands and added visual noise above the composer.
...
- **Motivation**: the sidebar's page-aware "新建任务" / "新建 Artifact" ("New Task" / "New Artifact") actions open a fresh chat that should land pre-filled with a starter prompt the user finishes typing (tasks/artifacts are created by talking to the AI).
- **Change**: the seed actions set `composerSeed` in the store *before*
  ...
  caret at the end and grows the textarea. Plain "新建对话" ("New conversation") leaves the seed null,
...
- **Motivation**: the three quick-suggestion chips on the welcome/empty state ("帮我关注金价" / "盯着孩子的作业" / "每天来份早报" — "Watch gold prices for me" / "Keep an eye on my kid's homework" / "Give me a daily digest") duplicate the four guide cards above, creating visual redundancy and diluting focus.
...
- **Motivation**: user feedback — clicking "新对话" ("New conversation") left the input unfocused; small interaction papercuts surfaced during a UI-detail review.
