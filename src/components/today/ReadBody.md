...
## Responsibility
Expanded body of a Today item: paragraphs + bullet lists, source line, and the action row (打开对话 / topic badge). Shared by list and card views.
...
- **Change**: removed the 标为已读/未读 toggle button; `ReadItemActions` now only carries `onOpenChat`.
