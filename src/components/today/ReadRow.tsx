...
export function ReadRow({ item, expanded, onToggleExpand, onOpenChat, onToggleRead }: ReadRowProps) {
  return (
    <div className={`nb-read-row${item.unread ? '' : ' read'}`}
      ...
      {expanded && <ReadBody item={item} onOpenChat={onOpenChat} onToggleRead={onToggleRead} />}
    </div>
  );
}
