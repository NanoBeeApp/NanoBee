...
- **Level 1 — open project** — the top row is a back affordance (a left arrow +
  ...
  (研究目录 — research outline, rendered by `ResearchOutlineTree`), so the rail
  mirrors the canvas structure and lets the user jump between nodes. Clicking the
  back row returns to Level 0.

Starting a new research project is the sidebar header's page-aware "new" button
(新建研究 — New Research), so this list never repeats that affordance.
...
- **Motivation**: the user wanted the canvas page's left rail to be a tree of the
  ...
  project list.
- **Goal**: show the open project's outline tree (research outline) on the canvas
  while keeping the saved-project switcher one toggle away.
...
- **Motivation**: the user found the outline ↔ project-list switch confusing.
  Each view had its own left-pointing "back" button ("← 研究项目" — ← Research
  Projects on the outline, "← 返回研究目录" — ← Back to Outline on the list), so
  it read as a back-stack where each side claimed the other was the parent — a
  directionless two-step loop.
